...
var zhuanxiangCountAll = 2;// 专项总次数
var zhuanxiangCount = 0;// 专项当前次数
...
function main()
{
    ...
    dailyQuestion();//每日答题
    zhuanxiangQuestion();//专项答题
    meizhouQuestion();//每周答题
...
ui.layout(
    <vertical>
        ...
        <button id="zxdt" text="专项答题"/>
        <button id="mxdt" text="每周答题"/>
...
function challengeQuestionLoop(conNum)
{
    ...
    if (className("ListView").exists()){
        ...
        let findListView =  className("ListView").findOnce();
        if(!findListView){
            return;
        }
        let listArray =findListView.children();//题目选项列表
        let i=random(0,listArray.length-1);
        console.log("随机点击一个");
        listArray[i].child(0).click();//随意点击一个答案
        return;
...
function getTipsStr(){
    ...
    while (tipsStr == "") {
        if (desc("查看提示").exists()) {           
            ...
            seeTips.click();
            console.log('点击提示:',clickCount);
            delay(2);
            click(device.width*0.5,device.height*0.51);
            delay(2);
            click(device.width*0.5,device.height*0.45);
            delay(2);
            clickCount++;
        } else {
        ...
        if (desc("提示").exists()) {
            ...
            //关闭提示
            click(device.width*0.5,device.height*0.21);
            delay(2);
            break;
        }
        if(clickCount>=3){
...
function getAnswerFromTips(timu, tipsStr) {
    console.log('从提示中获取填空题答案:',timu, tipsStr);
    var ansTips = "";
...
function zhuanxiangQuestion()
{
    ...
    delay(2);
    click("我要答题");
    while(!desc("专项答题").exists());
    delay(2);
    desc("专项答题").click();
    console.log("开始专项答题")
    delay(2);
    var jixu = desc("继续答题").findOnce();
    if(jixu){
        console.log("继续答题")
        jixu .click();
    }
    var kaishi = desc("开始答题").findOnce();
    if(kaishi){
        console.log("开始答题")
        kaishi .click();
    }
    ...
    while(true)
    {
        ...
        if(desc("再来一次").exists())
        {
            ...
            zhuanxiangCount++;
            // 返回了
            if(zhuanxiangCount<zhuanxiangCountAll){
                // 再进行一次
                zhuanxiangQuestion();
                break;
            }
            console.log("专项结束！返回主页！");
        }
    }
}
 ...
 * @return: null
 */
function zhuanxQuestionLoop()
{
    if(desc("填空题 (10分)").exists()){
        console.log('填空题 (10分)');
        var questionArray=getFitbQuestion();
    }
    else if(desc("多选题 (10分)").exists() || desc("单选题").exists()){
        console.log('多选题 (10分)');
        var questionArray=getChoiceQuestion();
    }
    var blankArray = [];
...
function meizhouQuestion()
{
    ...
    console.log("开始每周答题")
    delay(2);
    var weizuoda = desc("未作答").findOnce();
    if(weizuoda){
        weizuoda .click();
    }
    delay(2);
